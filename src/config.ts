...
export const config = {
  ...
  jwt: {
    secret: process.env.JWT_SECRET || (() => { if (process.env.NODE_ENV === 'production') throw new Error('FATAL: JWT_SECRET environment variable must be set in production'); return 'dev-only-insecure-secret'; })(),
...
export function getChainConfig(chainId: number): ChainConfig | null {
  if (chainId === config.chains.hoodi.chainId) return config.chains.hoodi;
  if (chainId === config.chains.baseSepolia.chainId) return config.chains.baseSepolia;
  return null;
}

export function getAllChainConfigs(): ChainConfig[] {
  return [config.chains.hoodi, config.chains.baseSepolia];
}
