...
import { formatPayoutForResponse } from '../utils/fees';
import { broadcastLeaderboardUpdate, notifyUser } from '../services/websocket';
...
router.get('/', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!['accuracy', 'volume'].includes(type as string)) {
      res.status(400).json({
        success: false,
 ...
 */
router.get('/user/:wallet', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const usersWithBetterAccuracy = await prisma.user.findMany({
      where: {
        totalPredictions: { gt: 0 },
      },
      select: {
    ...
    const usersWithBetterVolume = await prisma.user.count({
      where: {
 ...
 */
router.get('/reward-pool', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const totalVolume = await prisma.user.aggregate({
      _sum: {
        totalVolume: true,
      },
    });
    ...
    res.status(200).json({
      success: true,
      data: {
        poolBalance: formatPayoutForResponse(Math.max(0, poolBalance)),
        totalDistributed: formatPayoutForResponse(distributed),
        platformFeesCollected: formatPayoutForResponse(platformFeesCollected),
        updatedAt: new Date().toISOString(),
      },
    });
  } catch (error: any) {
 ...
 */
router.post('/rewards/claim', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Notify user
    notifyUser(user.walletAddress, {
      type: 'rewards_claimed',
      title: 'Rewards Claimed',
      message: `You claimed ${formatPayoutForResponse(claimableAmount)} in rewards`,
    ...
    res.status(200).json({
      success: true,
      data: {
        amountClaimed: formatPayoutForResponse(claimableAmount),
        transactionHash,
        newRewardsBalance: 0,
      },
    });
