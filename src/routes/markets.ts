...
import { calculateOdds, calculatePotentialPayout, calculateClaimablePayout } from '../utils/odds';
import { calculatePTF, calculatePayoutWithFee, formatPayoutForResponse } from '../utils/fees';
import {
...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const marketId = `MKT-${uuidv4().split('-')[0].toUpperCase()}`;
    ...
    broadcastGlobalEvent('market:created', {
...
router.get('/', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const marketsWithOdds = markets.map(market => {
      const odds = calculateOdds(market.yesStakes, market.noStakes);
      return {
        id: market.id,
        marketId: market.marketId,
        question: market.question,
 ...
 */
router.get('/:id/odds', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
 ...
 */
router.post('/:id/stake', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    const ptf = calculatePTF(amount);
    const netStakeAmount = ptf.net;

    // Calculate potential payout with current odds
    const payout = calculatePotentialPayout(netStakeAmount, outcome, market.yesStakes, market.noStakes);

    // Calculate odds at time of stake
    const currentOdds = calculateOdds(market.yesStakes, market.noStakes);
    const oddsAtStake = outcome === 'Yes' ? currentOdds.yesOdds : currentOdds.noOdds;

    const stakeIdOnchain = `STK-${uuidv4().split('-')[0].toUpperCase()}`;
    ...
    const newOdds = calculateOdds(updatedMarket.yesStakes, updatedMarket.noStakes);

    // Generate mock transaction hash
    const transactionHash = `0x${uuidv4().replace(/-/g, '')}${uuidv4().replace(/-/g, '').slice(0, 32)}`;

    // Broadcast updates
    broadcastNewStake(market.marketId, {
      marketId: market.marketId,
      stakeId: stake.id,
      outcome,
      amount: netStakeAmount,
 ...
 */
router.get('/:id/positions/:user', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
 ...
 */
router.post('/:id/resolve', authMiddleware, adminMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
 ...
 */
router.get('/:id/payout/:user', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id, user: walletAddress } = req.params;
    ...
    const claimable = calculateClaimablePayout(totalUserStake, winningPool, totalPot);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        claimableAmount: formatPayoutForResponse(claimable),
 ...
 */
router.post('/:id/payout/claim', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    // Notify user
    notifyUser(req.user!.walletAddress, {
      type: 'payout_claimed',
      title: 'Payout Claimed',
      message: `You claimed ${formatPayoutForResponse(claimable)} from market "${market.question}"`,
    ...
    res.status(200).json({
      ...
      data: {
        marketId: market.marketId,
        amountClaimed: formatPayoutForResponse(claimable),
